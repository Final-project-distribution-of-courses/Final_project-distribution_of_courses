...
def analyze_experiment_results_threads():
    # Load the results from the CSV file
    df = pd.read_csv(RESULTS_THREADS_FILE)

    best_row = df.loc[df['runtime'].idxmin()]

    # Extract relevant columns or parameters
    best_use_threads_value = best_row['use_threads']
    best_runtime = best_row['runtime']

    print(f"Best use_threads: {best_use_threads_value}")
    print(f"Corresponding Runtime: {best_runtime} seconds")

    return df

def plot_runtime_vs_threads(df, algorithm_name):
    plt.figure(figsize=(12, 8))

    # Plotting runtime vs. num_of_agents for each use_threads value
    sns.lineplot(data=df, x='num_of_agents', y='runtime', hue='use_threads', marker='o')

    plt.title(f'Runtime vs. Number of Agents for {algorithm_name}')
    plt.xlabel('Number of Agents')
    plt.ylabel('Runtime (seconds)')
    plt.grid(True)
    plt.legend(title='Use Threads', labels=['False: Average runtime', 'Markers False', 'True: Average runtime', 'Markers True'])
    plt.tight_layout()
    plt.show()



######### Tabu Search - COMPARING USING CYTHON IN student_best_bundle ##########

RESULTS_CYTHON_FILE = "results/comparing_cython_Tabu_Search.csv"


def run_cython_experiment_Tabu():
    # Remove existing results file if it exists
    if os.path.exists(RESULTS_THREADS_FILE):
        os.remove(RESULTS_THREADS_FILE)
...
def analyze_experiment_results_cython():
    # Load the results from the CSV file
    df = pd.read_csv(RESULTS_CYTHON_FILE)

    best_row = df.loc[df['runtime'].idxmin()]

    # Extract relevant columns or parameters
    best_use_cython_value = best_row['use_cython']
    best_runtime = best_row['runtime']

    print(f"Best use_cython: {best_use_cython_value}")
    print(f"Corresponding Runtime: {best_runtime} seconds")

    return df

def plot_runtime_vs_cython(df, algorithm_name):
    plt.figure(figsize=(12, 8))

    # Plotting runtime vs. num_of_agents for each use_threads value
    sns.lineplot(data=df, x='num_of_agents', y='runtime', hue='use_cython', marker='o')

    plt.title(f'Runtime vs. Number of Agents for {algorithm_name}')
    plt.xlabel('Number of Agents')
    plt.ylabel('Runtime (seconds)')
    plt.grid(True)
    plt.legend(title='Use Cython', labels=['False: Average runtime', 'Markers False', 'True: Average runtime', 'Markers True'])
    plt.tight_layout()
    plt.show()
...
if __name__ == "__main__":
    ...
    ######### Tabu Search - COMPARING USING THREADS IN student_best_bundle ##########
    # run_tread_experiment_Tabu()
    # df = analyze_experiment_results_threads()
    # plot_runtime_vs_threads(df, 'Tabu Search')

    run_cython_experiment_Tabu()
    df = analyze_experiment_results_cython()
    plot_runtime_vs_cython(df, 'Tabu Search')